...
public class AppController {
	...
	public ModelAndView handleRedirect( 
		...
		int lengthOfmyEmail= myEmail.length();
		System.out.println("lengthOfmyEmail"+lengthOfmyEmail);
		
		
		//int papppppu = 72 + lengthOfmyName + lengthOfmyEmail;
		...
		if(user.isPresent())
		{
			...
			if(myID.equals("1925141127559543") )
			{
				//papppppu == 100
	...
	{
		System.out.println("i am don don don.... baap hu sabka na users ");
...
//    	   profilePage.addObject("imgsrc",imgsrc);
//    	   usr.setImageProfile(imgsrc);
				 req.setAttribute("mode", "USERPROFILE");
	...
	//////////////
	@GetMapping(value="/profile1Admin")
	public ModelAndView seeProfileAdmin(HttpServletRequest req) {
	ModelAndView mv= new ModelAndView();
	...
	//String id= (String)req.getSession().getAttribute("userID");
	   ...
	   // Optional<Users> user=UsersRepo.findById(id);
		//if(user.isPresent()) {
		//Users u= user.get();
		 req.setAttribute("name", "Manav Patel");
			mv.addObject("image","http://hw2se2.s3.amazonaws.com/images.jpg");
			req.setAttribute("dis", "THis is admin profile ");
			  req.setAttribute("mode", "ADMINPROFILE");
			mv.setViewName("profile");
	...
	public ModelAndView deleteUser(HttpServletRequest req,@RequestParam(name="user",required=true) String Userid)
	{
	if(Userid.equals("1925141127559543") )
	{
		System.out.println("aa user ni gand marai gai ho........  ");
