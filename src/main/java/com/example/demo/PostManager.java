...
//		Friends fri=new Friends();
		...
		Post p=p3.get();
		if(uname.equals(p.getUname()))
		{
		System.out.println("aa ne thodi save karvanu hoi bc");	
		}
		else
		{
		noti1 noti11 = new noti1();
	    noti11.setCreatorName(p.getUname());
	    noti11.setCid(p.getUid());
		noti11.setPid(pid);
		noti11.setUid(ssid);
	...
	public ModelAndView seeThisPost(HttpServletRequest req,@RequestParam(name="pid",required=true) String pid) {
		...
		mv.addObject("Comments", ComeList);
		 req.setAttribute("mode","USER");
		mv.addObject("pid",pid);
		///////////////////////////////////////// this is to show photo and dis after adding commenets
		Optional<Post> p3=postrepo.findById(pid);
		Post p=p3.get();
	...
	@GetMapping(value="/seePostasAdmin")
	public ModelAndView seePostasAdmin(HttpServletRequest req, @RequestParam(name="pid",required=true) String pid) {
		ModelAndView mv= new ModelAndView();
...
//		Post p1= postrepo.findByPostID(pid);
	...
	@GetMapping(value="/seePostasUser")
	public ModelAndView seePostasUser(HttpServletRequest req, @RequestParam(name="pid",required=true) String pid) {
		ModelAndView mv= new ModelAndView();
...
//		Post p1= postrepo.findByPostID(pid);
	...
	public ModelAndView getnotification(HttpServletRequest req) {
		...
		////////////////////////////////////////////////////////////////////  if comment is posted
			...
			List<noti1> noti1= notirepo1.findByCid(uid);
			System.out.println("for comment nottttttttification  == "+noti1.size());
			mv.addObject("NOTE1", noti1);
